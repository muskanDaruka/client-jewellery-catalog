...
const BraceletDisplay = () => {
  return (
    <div className="bg-[#e1e1db] h-screen flex flex-col xl:justify-center items-center font-montserrat px-4 py-8">
      <h2 className="text-2xl font-bold text-center mb-8">DIAMOND ATELIER</h2>
      <div className="grid grid-cols-3 gap-6 w-full max-w-5xl items-start">
        <div className="flex flex-col items-center bg-[#faf7f1] xl:p-6 p-2 rounded-lg shadow-md w-full">
          <Image
            src={bracelet}
            alt="Emerald Bracelet"
            width={200}
            height={400}
            className="w-full max-w-xs h-full object-cover rounded-md"
          />
          <p className="mt-3 xl:text-lg text-sm font-semibold">Emerald Bracelet</p>
        </div>
            ...
            <tbody>
              {["Diamond Weight", "Metal Type", "Metal Weight", "Price"].map((item, index) => (
                <tr key={index} className="border-b last:border-none border-[#e1e1db]">
                  <td className="p-3 font-medium border-r border-[#e1e1db] text-xs md:text-base text-gray-700">
                    {item}
                  </td>
